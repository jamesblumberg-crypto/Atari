ff; test
DOOR_NORTH              = %1000
DOOR_SOUTH              = %0100
DOOR_WEST               = %0010
DOOR_EAST               = %0001

MAP_WALL                = 1
MAP_FLOOR               = 127
MAP_UP                  = 121
MAP_DOWN                = 57
MAP_DOOR                = 4
MAP_DOORWAY             = 59

; Item tiles (must be >= PASSABLE_MIN / outside monster range 44-51)
MAP_BOW                 = 56    ; Bow pickup item (uses chars 112-113)
; Map gem tiles reuse dungeon gem icon left-halves (same glyphs as the HUD)
MAP_GEM_WHITE           = 34    ; map id is charset index / 2 because of blit_tile’s asl
MAP_GEM_BLACK           = 35
MAP_GEM_BLUE            = 36
MAP_GEM_RED             = 37
MAP_GEM_GOLD            = 38

; Well (dungeon charset chars 22-25 = two map tiles: 11 and 12)
MAP_WELL_L              = 11    ; chars 22-23
MAP_WELL_R              = 12    ; chars 24-25

; Keys (inventory bits)
KEY_BLUE                = %00000001
KEY_RED                 = %00000010
KEY_GOLD                = %00000100
KEY_WHITE               = %00001000
KEY_BLACK               = %00010000
KEY_ALL                 = %00011111

; Floor key items (drawn via blit_one_tile special cases; not plain asl pairs)
MAP_KEY_BLUE            = 6     ; boss drop only
MAP_KEY_RED             = 7
MAP_KEY_GOLD            = 8
MAP_KEY_WHITE           = 9
MAP_KEY_BLACK           = 10

; Floor-4 stationary boss: 2x2 map tiles (each tile = 2 chars in live slots 88-95)
MAP_BOSS_TL             = 44
MAP_BOSS_TR             = 45
MAP_BOSS_BL             = 46
MAP_BOSS_BR             = 47

; Inventory bitfield (has_gems)
GEM_BLUE                = %00000001
GEM_GOLD                = %00000010
GEM_RED                 = %00000100
GEM_BLACK               = %00001000
GEM_WHITE               = %00010000
GEM_ALL                 = %00011111

; Frame lines. Vert=1, horiz=4 (solid blue+white bar).
; Status + bottom trailing antic4 use outdoor CHBASE (corners 10-15).
; Playfield uses dungeon (keys at 10-18); dungeon 106-111 still hold corner copies.
UI_VERT_BORDER          = 1
UI_HORIZ_BORDER         = 4
UI_TOP_TEE              = 10    ; outdoor native
UI_NW_BORDER            = 12    ; outdoor native
UI_NE_BORDER            = 13    ; outdoor native
UI_BOTTOM_TEE_OUT       = 11    ; outdoor native
UI_SW_BORDER_OUT        = 14    ; outdoor native
UI_SE_BORDER_OUT        = 15    ; outdoor native

; Key icons (chars 10-18) — HUD and floor keys via blit_one_tile (dungeon CHBASE)
UI_WHITE_KEY_ICON		= 10
UI_KEY_ICON_RIGHT		= 11
UI_BLUE_KEY_ICON		= 12
UI_RED_KEY_ICON			= 13
UI_GOLD_KEY_ICON		= 14 + 128
UI_BLACK_KEY_CAP_LEFT	= 15
UI_BLACK_KEY_ICON_LEFT	= 16
UI_BLACK_KEY_ICON_RIGHT	= 17
UI_BLACK_KEY_CAP_RIGHT	= 18
; Merged Magic Key HUD glyph (gold key art + color bit)
UI_MAGIC_KEY_ICON		= 14 + 128

UI_MELEE_ICON_LEFT 		= 26
UI_MELEE_ICON_RIGHT 	= 27
UI_DEFENSE_ICON_LEFT 	= 28
UI_DEFENSE_ICON_RIGHT 	= 29
UI_RANGED_ICON_LEFT 	= 30
UI_RANGED_ICON_RIGHT 	= 31
UI_FORTITUDE_ICON_LEFT 	= 32
UI_FORTITUDE_ICON_RIGHT = 33
UI_TORCH_ICON_LEFT		= 34 + 128
UI_TORCH_ICON_RIGHT		= 35 + 128
UI_COIN_ICON_LEFT		= 36 + 128
UI_COIN_ICON_RIGHT		= 37 + 128
UI_POTION_ICON_LEFT		= 38
UI_POTION_ICON_RIGHT	= 39
UI_HP_ICON_LEFT 		= 40
UI_HP_ICON_RIGHT 		= 41
UI_XP_ICON_LEFT 		= 42
UI_XP_ICON_RIGHT 		= 43
UI_NUMBER_0				= 44
UI_NUMBER_1				= 45
UI_NUMBER_2				= 46
UI_NUMBER_3				= 47
UI_NUMBER_4				= 48
UI_NUMBER_5				= 49
UI_NUMBER_6				= 50
UI_NUMBER_7				= 51
UI_NUMBER_8				= 52
UI_NUMBER_9				= 53
UI_BAR_LEFT 			= 54
UI_BAR_RIGHT 			= 55
UI_BAR_EMPTY			= 56
UI_COLON				= 57
UI_HP_QTR				= 58
UI_HP_HALF				= 59
UI_HP_3_QTR				= 60
UI_HP_FULL 				= 61
UI_XP_QTR				= 62
UI_XP_HALF				= 63
UI_XP_3_QTR				= 64
UI_XP_FULL 				= 65

UI_BLANK_GEM_ICON_LEFT	= 66
UI_BLANK_GEM_ICON_RIGHT = 67
UI_WHITE_GEM_ICON_LEFT	= 68
UI_WHITE_GEM_ICON_RIGHT = 69
UI_BLACK_GEM_ICON_LEFT	= 70
UI_BLACK_GEM_ICON_RIGHT = 71
UI_BLUE_GEM_ICON_LEFT	= 72
UI_BLUE_GEM_ICON_RIGHT 	= 73
UI_RED_GEM_ICON_LEFT	= 74
UI_RED_GEM_ICON_RIGHT 	= 75
UI_GOLD_GEM_ICON_LEFT	= 76 + 128
UI_GOLD_GEM_ICON_RIGHT 	= 77 + 128
UI_AMULET_NW_ICON_LEFT	= 78
UI_AMULET_NW_ICON_RIGHT	= 79
UI_AMULET_NE_ICON_LEFT	= 80
UI_AMULET_NE_ICON_RIGHT	= 81
UI_AMULET_SW_ICON_LEFT	= 82
UI_AMULET_SW_ICON_RIGHT	= 83
UI_AMULET_SE_ICON_LEFT	= 84
UI_AMULET_SE_ICON_RIGHT	= 85

PASSABLE_MIN          = 55

STICK_UP    = %0001
STICK_DOWN  = %0010 
STICK_LEFT  = %0100
STICK_RIGHT = %1000

BUTTON_DOWN = 0
BUTTON_UP   = 1

NORTH                   = 1
SOUTH                   = 2
WEST                    = 3
EAST                    = 4
